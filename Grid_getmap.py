...
from model.model import GridDehazeNet
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------------------#
    classes_path = 'model_data/voc_Grid_classes.txt'
    # --------------------------------------------------------------------------------------#
    ...
    # -------------------------------------------------------#
    attack_map_out_path = "fine-tuning_haze12.30"

    txt = open(os.path.join("voc_norm_test.txt")).readlines()
    ...
    if map_mode == -1:
        ...
        # --- Define the network --- #
        net = GridDehazeNet(height=3, width=6, num_dense_layer=4,
                            growth_rate=16)
        ...
        net.load_state_dict(torch.load('fine-tuning_haze_3_6'))
        net.eval()
        ...
        for image_id in tqdm(image_ids):
            ...
            for i in range(2):
                s = (5 + i)/100
                Grid_image = "".join((image_name,"_",str(s)))
                image_path = os.path.join(VOCdevkit_path, "VOC2007/test/" + Grid_image + ".jpg")
                ...
                a = image.resize((nw, nh), Image.BICUBIC)
                ...
                for j in range(len(new_image)):
                    clear_image = torchvision.transforms.ToPILImage()(new_image[j])
                    yolo.get_map_txt(Grid_image, clear_image, class_names, attack_map_out_path, np.array(np.shape(image)[0:2]))
    ...
    if map_mode == -1:
        ...
        for image_id in tqdm(image_ids):
            ...
            for i in range(2):
                s = (5 + i) / 100
                with open(os.path.join(attack_map_out_path, "ground-truth/" + image_name +"_" +str(s)+".txt"), "w") as new_f:
    ...
    if map_mode == 0:
        ...
        for image_id in tqdm(image_ids):
            ...
            for i in range(2):
                ...
                image_path = os.path.join(VOCdevkit_path, "VOC2007/test/" + Grid_image + ".jpg")
                image = Image.open(image_path)
                yolo.get_map_txt(Grid_image, image, class_names, map_out_path)
    ...
    if map_mode == 0:
        ...
        for image_id in tqdm(image_ids):
            ...
            for i in range(2):
                s = (5 + i) / 100
                with open(os.path.join(map_out_path, "ground-truth/" + image_name +"_" +str(s)+".txt"), "w") as new_f:
                    root = ET.parse(os.path.join(VOCdevkit_path, "VOC2007/Annotations/" + image_name + ".xml")).getroot()
                    for obj in root.findall('object'):
                        difficult_flag = False
                        if obj.find('difficult') != None:
