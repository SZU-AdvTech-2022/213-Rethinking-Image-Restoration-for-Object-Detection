...
if __name__ == "__main__":
    ...
    #-------------------------------------------------------#
    attack_map_out_path    = 'attack_map_out'
    ...
    if map_mode == -1:
        ...
        for image_id in tqdm(image_ids):
            ...
            attack_image_path  = os.path.join(VOCdevkit_path, "VOC2007/attack_JPEGImages/attack_"+image_id+".jpg")
            attack_image       = Image.open(attack_image_path)
            image       = Image.open(image_path)
            yolo.get_map_txt(image_id, attack_image, class_names, attack_map_out_path, np.array(np.shape(image)[0:2]))

    if  map_mode == -1:
        print("Get ground truth result.")
        for image_id in tqdm(image_ids):
            with open(os.path.join(attack_map_out_path, "ground-truth/"+image_id+".txt"), "w") as new_f:
